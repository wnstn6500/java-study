...
//	좌석 정렬 후: [A1, A2, A3, B1, B2]
	...
	public static void main(String[] args) {
		...
		String reservationSeats = sc.nextLine();
		
		if (seats.contains(reservationSeats)) {
			seats.set(seats.indexOf(reservationSeats), reservationSeats+"(예약)");
			System.out.println("좌석 예약 후"+seats);
		} else {
			System.out.println(reservationSeats+"은(는) 존재하지 않는 좌석입니다.");
			System.out.println("좌석 예약 후"+seats);
		}
		System.out.print("삭제 좌석: ");
		String deleteSeats = sc.nextLine();
		
		if (seats.contains(deleteSeats)) {
			seats.remove(deleteSeats);
			System.out.println("좌석 삭제 후 : "+seats);
		} else {
			System.out.println(deleteSeats+"좌석은 목록에 없습니다.");
			System.out.println("좌석 삭제 후 : "+seats);
		}
